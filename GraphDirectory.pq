...
scopes = {
    "Directory.Read.All"
    // add additional scopes here in format:
    // "Contacts.Read",
    //  "User.ReadBasic.All",
    //  "Calendars.ReadWrite",
    //  "Mail.ReadWrite",
...
// Exported function(s)
//
[DataSource.Kind="GraphDirectory", Publish="GraphDirectory.UI"]
shared GraphDirectory.Contents = () =>
    let
        source = OData.Feed("https://graph.microsoft.com/v1.0/users?%24select=id,displayName,mail,userPrincipalName,userType", null, [ ODataVersion = 4, MoreColumns = true ])
    in
        source;
...
//
GraphDirectory = [
    TestConnection = (dataSourcePath) => { "GraphDirectory.Contents" },
    Authentication = [
        OAuth = [
    ...
    Label = "Azure Directory Connector"
];
...
//
GraphDirectory.UI = [
    ButtonText = { "GraphDirectory.Contents", "Connect to Graph" },
    SourceImage = GraphDirectory.Icons,
    SourceTypeImage = GraphDirectory.Icons
];

GraphDirectory.Icons = [
